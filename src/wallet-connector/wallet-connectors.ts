...
export class StellarWalletsKitAdapter implements WalletAdapter {
    ...
    async connect(): Promise<ConnectedWallet> {
        throw new WalletError('Please use connectWallet(walletType) method instead');
    }
    ...
    async connectWallet(walletType: WalletType): Promise<ConnectedWallet> {
        try {
            ...
            throw new WalletError(`Failed to connect ${getWalletDisplayName(walletType)}: ${errorMessage}`);
        }
    }
    ...
    async signTransaction(
        ...
        try {
            const networkPassphrase = opts?.networkPassphrase || kitNetworkToStellarNetwork(this.network);
    ...
    async signMessage?(message: string): Promise<string> {
        ...
        try {
            ...
            throw new WalletError('Message signing not implemented in Stellar Wallets Kit');
        } catch (error) {
    ...
    async createButton(params: {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to create button';
...
// 创建钱包适配器
export function createWalletAdapter(network: Networks = Networks.TESTNET): StellarWalletsKitAdapter {
    if (!globalAdapter) {
...
// 向后兼容的方法
export function connectWallet(walletType: WalletType, network: Networks = Networks.TESTNET): Promise<ConnectedWallet> {
