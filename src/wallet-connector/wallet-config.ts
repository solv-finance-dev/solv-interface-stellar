...
export function createStellarWalletsKit(
...
export async function getAvailableWallets(
  ...
  return SUPPORTED_WALLETS.map(wallet => {
    const supportedWallet = supportedWallets.find(
      (sw: ISupportedWallet) => getWalletIdFromType(wallet.id) === sw.id
    );
...
export async function isWalletInstalled(
  ...
  const supportedWallets = await kit.getSupportedWallets();
  const supportedWallet = supportedWallets.find(
    (sw: ISupportedWallet) => sw.id === walletId
  );
...
// 验证钱包类型
export function isValidWalletType(
  walletType: string
): walletType is WalletType {
